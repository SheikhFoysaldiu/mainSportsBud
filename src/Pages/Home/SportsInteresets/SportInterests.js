...
const SportInterests = () => {
  ...
  const fetchSports = async ({ pageParam = 1 }) => {
    ...
    const res = await fetch(
      `${API_URL}/api/v1/sport/sports?page=${pageParam}&limit=${10}&searchQuery=${search}`,
      {
        method: "GET",
        headers: {
  ...
  const {
    ...
    // gettting sports at max 10 per page
  ...
  if (data.pages[0].data.length === 0 && data.pages.length == 1) {
    return <p className="text-center">No Sports Found!!</p>;
  }
